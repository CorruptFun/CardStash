...
Keep them small, but not smaller than the pipeline can use — and the right
size differs by KIND. `ingest.mjs` applies it for you: **1600px** long edge for
a single card (`CAPTURE_MAX_EDGE`, what `captureFrame` hands `identifyFrame`),
**3200px** for a binder page (`PAGE_MAX_EDGE`, because a page is cut into ~9
crops before anything reads it — at 1600 each card lands ~370px wide, under
the ~790px where a printed collector line stops being legible).

**Send originals to `ingest.mjs`, don't pre-resize.** A resize you did by hand
cannot be undone, and the caps above are code that may move.
