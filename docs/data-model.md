...
| `cloudScanRescue` (`false`) / `rescueAutoOnAt` (`0`) | The consent switch for the cloud scan rescue, and the one-shot marker behind its auto-on: the first time this device sees an active subscription, `noteEntitlementSeen()` (`lib/billing.ts`) switches the rescue on and stamps the marker — never again, so turning it off afterwards is final. The marker is **not** cleared on sign-out: the flip is per-device, not per-account. |
...
| `scanOfferAt` (`0`) | When the scan screen's subscription offer was last waved off. Persisted rather than kept in view state because the offer is made at a bad moment by definition — a card is refusing to read — and a "no thanks" that lasts until the next launch is a delay, not an answer. A run of misses is also the easiest thing in this app to reproduce by accident. 14 days (`SCAN_OFFER_SNOOZE_MS`). |
