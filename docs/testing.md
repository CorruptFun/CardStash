...
| `authsession.test.mjs` | The session surviving its own app being busy — and its own second tab. Refresh tokens rotate, so the concurrent refreshes one screen-load fires must coalesce (the losers' 400s used to `signOut()` a real returning user), a refresh must spend the token **storage** holds rather than a memoized one, and an "Already Used" rejection must be re-checked against storage before it ends anything — two tabs used to sign each other out. Also that a 500/timeout never does, that tabs see each other's sign-out, and which store "Keep me signed in" puts the tokens in. |
