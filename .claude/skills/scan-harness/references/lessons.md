...
58. **A grader that asks only for the NAME cannot see the failure a user
...
59. **Every corner region is mapped through the DETECTED card region, so a crop
...
60. **The raw-frame fallback existed, was left-handed, and had never once
    run.** `readCornerInfo`'s thorough tier already carried the escape hatch
    for lesson 59 — a band of the raw frame rather than of the card region —
...
61. **Pokémon's edition is decided by the collector line, not refined by it.**
...
62. **The two halves of a Pokémon fraction fail INDEPENDENTLY, so the guard
...
63. **A fix that measures as no-change may still be two bugs deep.** The first
    ...
    (the wrong way, lesson 62) and the trace said the new passes had run but
    the ones that mattered had not (lesson 60). Diff the CELLS, not the
