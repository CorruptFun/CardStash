...
/**
 * The artwork window of an upright card, as fractions of the card: the band
 * below the title and above the text box on every standard frame. Shared by
 * the capture side (a card crop from the camera) and the catalog side (the
 * sync worker hashing set images), which is the whole point — two hashes are
 * only comparable because both sides cut the same window.
 */
export const ART_REGION = { x: 0.1, y: 0.18, w: 0.8, h: 0.37 } as const

/** 17 columns give 16 gradient comparisons per row; 16 rows → 256 bits. */
const ART_GRID_W = 17
const ART_GRID_H = 16
export const ART_HASH_BITS = (ART_GRID_W - 1) * ART_GRID_H

/**
 * Pack a 17×16 luma grid into a 256-bit horizontal-gradient hash (64 hex
 * chars). Gradient-only on purpose — no mean bits, no mixing: a phone capture
 * and a flat catalog scan disagree wildly about absolute brightness (exposure,
 * sheen, white balance) but agree about which of two neighbouring patches is
 * darker, and folding a mean bit in (as the first cut of this did, XORed per
 * pixel) would carry that lighting noise into every bit position.
 *
 * Pure — `gray` is any row-major array of 17×16 luma values — so node tests
 * exercise it without a canvas.
 */
export function artHashFromGray(gray: ArrayLike<number>): string {
  let hex = ''
  let nibble = 0
  let bits = 0
  for (let y = 0; y < ART_GRID_H; y++) {
    for (let x = 0; x < ART_GRID_W - 1; x++) {
      const at = y * ART_GRID_W + x
...
/**
 * 256-bit perceptual fingerprint of a card's artwork region, for telling
 * printings of the SAME card apart — alternate art, full art, retro frame.
 * `sourceWidth`/`sourceHeight` are the source's intrinsic pixel size (a canvas
 * passes its own; an ImageBitmap of a catalog image passes the image's).
 *
 * It may only ever choose BETWEEN printings of an already-identified card,
 * exactly like the cloud read's `treatment` — art similarity is nowhere near
 * evidence enough to name a card. `pickPrintingByArt` in catalog.ts holds the
 * thresholds; measured numbers live with them.
 */
export function cardArtHash(source: CanvasImageSource, sourceWidth: number, sourceHeight: number): string {
  const ctx = scaledContext(ART_GRID_W, ART_GRID_H)
  ctx.drawImage(
    source,
    Math.floor(sourceWidth * ART_REGION.x),
    Math.floor(sourceHeight * ART_REGION.y),
  ...
  return artHashFromGray(grayscale(ctx.getImageData(0, 0, ART_GRID_W, ART_GRID_H)))
}
 ...
 * hashes is a plausible real distance, and "malformed" must never outrank a
 * genuine candidate.
 */
export function artHashDistance(a: string, b: string): number {
