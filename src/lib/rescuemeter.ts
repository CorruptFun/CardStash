...
/** How long a dismissed miss-screen offer stays away. */
export const RESCUE_HINT_QUIET_MS = 14 * 86_400_000

/**
 * Has the "cloud rescue could read this" offer been waved off recently? The
 * offer returns after a fortnight — misses keep happening and the state that
 * made it true hasn't changed — but never sooner: an offer that reappears on
 * the next miss is a nag, and the free path is promised none.
 */
export function rescueHintQuiet(dismissedAt: number, at: number = Date.now()): boolean {
  return typeof dismissedAt === 'number' && dismissedAt > 0 && at - dismissedAt < RESCUE_HINT_QUIET_MS
}
